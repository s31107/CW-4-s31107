...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps, emp1 => emp1.Mgr, emp2 => emp2.EmpNo, 
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Select(emp => new { emp.EName, emp.DeptNo, Grade=grades.Where(
            sal => sal.Losal <= emp.Sal && sal.Hisal >= emp.Sal) }).SelectMany(
            item => item.Grade, (group, grade) => 
                new { group.EName, group.DeptNo, grade.Grade }).Join(depts, emp => emp.DeptNo, 
            dept => dept.DeptNo, (emp, dept) => new { emp.EName, dept.DName, emp.Grade }); 
